...
class EvaluationResult(StrictBaseModel):
    ...
    critique: str = Field(
        min_length=1,
        description="Concrete explanation of what is good or insufficient about the current output.",
    )
...
class AssetCatalog(StrictBaseModel):
    ...
    available_stac_images: list[StacImageCatalogEntry] = Field(
        default_factory=list,
        description="Per-item STAC catalog rows with acquisition date, asset key, and cloud cover for the current query constraints.",
    )
...
class GeoLocation(StrictBaseModel):
    ...
    bbox_wgs84_lat_lon: BoundingBox | None = Field(
        default=None,
        description="Optional WGS84 bounding box describing the spatial extent of the location.",
    )
    time_range: TimeRange | None = Field(
        default=None,
        description="Optional time range resolved alongside the location for downstream EO queries.",
    )
    source: str = Field(
...
class DataRequest(StrictBaseModel):
    ...
    max_cloud_cover: float | None = Field(
        ...
        ge=0.0,
        le=100.0,
        description="Maximum acceptable cloud coverage percentage (range 0..100) for candidate EO scenes.",
    )
    selection_reasons: list[str] = Field(
        default_factory=list,
        description="Short justifications explaining why the selected imagery parameters were chosen.",
    )

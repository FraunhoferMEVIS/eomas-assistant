...
class LLMHelper:
    """Shared helpers for robust response handling."""
    ...
    def get_latest_user_message(messages: Sequence[AnyMessage]) -> str:
        ...
        for message in reversed(messages):
            if not isinstance(message, HumanMessage):
                continue
            content = LLMHelper.get_plain_text(message)
            return content
        return ""
    ...
    def build_model_input_messages(
        ...
        input_messages: list[AnyMessage] = [
            SystemMessage(
                content=LLMHelper.build_system_prompt_with_current_date(system_prompt)
            ),
            *list(messages),
        ]
        ...
        for message in input_messages:
            logger.debug(
                ...
                LLMHelper.describe_message_content(message),
            )
    ...
    def call_llm_with_schema(
        ...
        for attempt in range(LLMHelper._MAX_RETRY_ATTEMPTS):
            try:
                logger.info(
                ...
                retryable = LLMHelper._is_retryable_structured_call_error(exc)
                logger.warning(
                ...
                if attempt + 1 >= LLMHelper._MAX_RETRY_ATTEMPTS or not retryable:
                    raise
    ...
    def _is_retryable_structured_call_error(exc: Exception) -> bool:
        ...
        if isinstance(exc, httpx.HTTPStatusError):
            return exc.response.status_code in LLMHelper._RETRYABLE_STATUS_CODES
    ...
    def describe_message_content(ai_message: AnyMessage) -> str:
        ...
        return LLMHelper.get_plain_text(ai_message) or "[no content]"
    ...
    def stream_text(
        ...
        """Yield plain-text fragments from a token-streaming LLM response."""
        ...
        for chunk in llm.stream(input=llm_input):
            text = LLMHelper.get_plain_text(chunk)
            if text:
                yield text
